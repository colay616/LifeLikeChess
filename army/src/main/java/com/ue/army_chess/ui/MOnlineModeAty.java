...
public class MOnlineModeAty extends BaseOnlineModeAty {
    ...
    public void selectRoomLevel() {
        final int curRoomFlag = XSharedPref.getInt(GameConstants.ROOM_LEVEL_ACM, GameConstants.ROOM_LEVEL_PRIMARY);
        showSelectRoomDialog(curRoomFlag, new DialogOnItemClickListener() {
            ...
            public void onItemClick(Button button, int position) {
                ...
                //保存当前选择的级别
                XSharedPref.putInt(GameConstants.ROOM_LEVEL_ACM, newLevel);
                //退出当前房间
                ...
                //加入新房间
                roomId = newLevel == GameConstants.ROOM_LEVEL_PRIMARY ? GameConstants.ROOM_ACM_1 : GameConstants.ROOM_ACM_2;
    ...
    private void initLayoutPopupWin() {
        mLayoutPopupWin = new LayoutPopupWin(MOnlineModeAty.this);
        mLayoutPopupWin.setLayoutItemClicker(new LayoutPopupWin.OnLayoutItemClick() {
            ...
            public void onEditClick(LayoutItem layoutItem) {
                Intent intent = new Intent(MOnlineModeAty.this, EditLayoutAty.class).putExtra("targetItem", layoutItem);
    ...
    public boolean checkCanStart() {
        ...
        if (TextUtils.isEmpty(opponentUserName)) {
            GameDialogs.showTipDialog(MOnlineModeAty.this, "正在寻找另一半，请稍等...");
            return false;
        }
        if (null == myChessList) {
