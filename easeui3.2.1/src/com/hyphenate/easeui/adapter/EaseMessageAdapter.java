...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    Handler handler = new Handler() {
        private void refreshList() {
            ...
            ////////////不显示游戏聊天消息
            for(int len=var.size(),i=len-1;i>0;i--){
                EMMessage itemMsg=var.get(i);
                if (itemMsg.getChatType() == EMMessage.ChatType.Chat && !TextUtils.isEmpty(itemMsg.getStringAttribute(GameConstants.ATTR_GAME_CHAT, null))) {
                    var.remove(i);
                }
            }
            ////////////
