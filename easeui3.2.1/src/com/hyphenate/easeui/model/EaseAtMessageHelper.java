    ...
    public void parseMessages(List<EMMessage> messages) {
        ...
        for (EMMessage msg : msgs) {
            if (msg.getChatType() == ChatType.GroupChat) {
                ...
                //作为版本更新的第二种方式,第一种是bmob云端代码
                if (msg.getBooleanAttribute("isUpdate", false)) {
                    int newVerCode = msg.getIntAttribute(PreferenceManager.NEW_VER_CODE, -1);
                    String newVerName = msg.getStringAttribute(PreferenceManager.NEW_VER_NAME, "");
                    String newVerUrl = msg.getStringAttribute(PreferenceManager.NEW_VER_URL, "");
                    String newVerDescribe = msg.getStringAttribute(PreferenceManager.NEW_VER_DESC, "");
